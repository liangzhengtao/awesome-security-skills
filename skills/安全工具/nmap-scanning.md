...
1. **主机发现**：Ping 扫描（`-sn`）识别存活主机，使用 ARP 扫描（本地网络最可靠）、TCP SYN ping、ICMP echo ping 等方式。
2. **端口扫描**：快速初始扫描（Top 1000 端口），全面扫描（全部 65535 端口 `-p-`），UDP 扫描（Top 200），根据环境选择合适的时间模板（`-T3` 生产环境，`-T4` 受控环境）。
3. **服务枚举**：版本检测（`-sV`），针对特定服务运行 NSE 脚本（HTTP 枚举、匿名 FTP、SMB 共享、SMTP 用户枚举等）。
4. **NSE 脚本扫描**：使用 `--script vuln` 做漏洞检查，`--script safe` 做非侵入式扫描，按类别（auth、brute、discovery、vuln 等）选择脚本。
5. **OS 检测与路由追踪**：`-O` 指纹识别操作系统，`--traceroute` 发现网络路径，`-A` 组合扫描。
6. **防火墙绕过**：分片数据包（`-f`）、诱饵扫描（`-D RND:10`）、源端口操纵（`--source-port 53`）、Idle/zombie 扫描。
7. **输出与报告**：使用 `-oA` 同时保存所有格式（Normal、XML、Grepable），便于后续处理。

### 模板说明

- **综合评估扫描脚本**：自动化 Bash 脚本，依次执行主机发现、快速端口扫描、全端口扫描、服务枚举、漏洞扫描、OS 检测和路由追踪，结果分阶段保存。
- **扫描结果报告模板**：标准化 Markdown 格式，包含扫描参数、主机摘要、每主机端口/服务表和关键发现。

### 常见陷阱

- **未授权扫描** — 未经授权的端口扫描在大多数司法管辖区属于非法行为，必须获取书面许可。
- **在生产环境使用激进时间模板** — `-T5` 或高并行度可能导致服务崩溃或网络饱和，生产系统使用 `-T3`。
- **不扫描全部端口** — 默认扫描仅覆盖 Top 1000 端口，非常见端口（8443、9090、27017）上的服务会被遗漏。
- **忽略 UDP** — UDP 服务（DNS、SNMP、TFTP、NTP）常被忽视但可能是关键攻击向量，至少扫描 Top 100 UDP 端口。
- **不验证发现结果** — Nmap 的版本检测和脚本输出可能不准确，必须手动验证。
- **不保存扫描输出** — 始终使用 `-oA` 保存结果，重跑全量扫描非常耗时。
- **忘记重新扫描** — 网络会变化，防火墙规则变更、服务器部署后或每次测试开始时应重新扫描。
